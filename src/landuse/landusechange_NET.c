...
/**                  +-+-+-+-+-+-+-+-+-+-+                        **/
/**                                                               **/
/**  src/landuse/  l a n d u s e c h a n g e _ N E T . c          **/
/*******************************************************************/
#include "lpj.h"
#include "tree.h"
#include "grass.h"
...
void deforest(Cell *cell,Real difffrac,const Pftpar pftpar[],Landuse *landuse,int npft,int year,Config config)
{
  ...
  if(existlandusetype(cell->standlist,&s,PRIMARY)){
    ...
    reclaim_land(natstand,cutstand,&harvestdef,&current_deforest_product); 
    ...
    setaside(cell,&pos,pftpar,landuse->intercrop,npft,TRUE);
  }
               ...
               int ntypes, int year,Config config)
{
  ...
  setasidestand=getstand(cell->standlist,s);
    ...
    if(setasidestand->frac<=difffrac*-1+epsilon) { /*setaside stand has not enough space for regrowth*/
      ...
      reclaim_land(setasidestand,mixstand,&dummy,&current_deforest_product); 
            ...
            Stand *grassstand,int year,Config config)
{
  ...
  if(grassstand->frac<=difffrac+epsilon){
    ...
    setaside(cell,s,pftpar,landuse->intercrop,npft,TRUE);
  }else{
    pos=addstand(cell->standlist);
    pos--;
    cutstand=getstand(cell->standlist,pos);
    ...
    reclaim_land(grassstand,cutstand,&dummy,&current_deforest_product);
...
void landusechange_NET(Cell *cell,
       const Pftpar *pftpar,
       int npft,
       int ncft,
       int ntypes,
...
/*needs to be called before establishment, to ensure that regrowth is possible in the following year*/
{
  ...
  difffrac=newcropfrac-cell->cropfrac; //newFrac - oldFrac

  //assumption #1: difffrac > 0.001
  //assumption #2: difffrac > 0.000001
  ...
  else if(difffrac < -0.001) regrowth(cell,difffrac,pftpar,npft,ntypes,year,config); // Regrowth only if SETASIDE stand exists

  //---------------------------------------------------------------------
  //    L a n d  U s e  C h a n g e :  N e t   T r a n s i t i o n s 
  //---------------------------------------------------------------------
  if (difffrac != 0){
    ...
    if (findstand(cell->standlist,&s,GRASSLAND,FALSE)){
      ...
      difffrac=stand->frac-grassfrac[0]; //oldFrac - newFrac

      if(difffrac> 0.000001)
        grasslandreduction(cell,difffrac,pftpar,landuse,npft,&s,stand,year,config);
    ...
    if (findstand(cell->standlist,&s,GRASSLAND,TRUE)){
      ...
      if(difffrac> 0.000001)
        ...
        landexpansion(cell,difffrac,pftpar,npft,ntypes,exist,stand,irrigation,cultivation_type,year,config);
    }else if(grassfrac[1] > 0.000001){
      exist=FALSE;
      difffrac=grassfrac[1]*-1;
      landexpansion(cell,difffrac,pftpar,npft,ntypes,exist,dummy,irrigation,cultivation_type,year,config);
    }
  }
  // end LUC Net Transitions
  //---------------------------------------------------------------------
