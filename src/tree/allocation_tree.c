...
/**                  +-+-+-+-+-+-+-+-+-+-+                        **/
/**                                                               **/
/**  src/tree/ a l l o c a t i o n _ t r e e . c                  **/
/*******************************************************************/
#include "lpj.h"
#include "tree.h"
...
void allocation_tree(Litter *litter,Pft *pft,Real *fpc_inc,Bool subannual)
{
  ...
  { 
        ...
        tinc_ind_min.root+tinc_ind_min.leaf<=bm_inc_ind) || bm_inc_ind<=0.0) 
    {
      ...
      if((x1==0 && x2==0)       || 
          data.b-x1*data.lm<0.0 || 
          data.ind_leaf+x1<=0.0 || 
          data.b-x2*data.lm<0.0 || 
          data.ind_leaf+x2<=0.0 )
        tinc_ind.leaf=0;
      else
    ...
    else 
    {
      ...
      */
      tinc_ind.leaf=(bm_inc_ind+tree->ind.root-tree->ind.leaf/lmtorm)/ (1.0+1.0/lmtorm);
      if (tinc_ind.leaf>0.0) 
      {
      ...
      else 
      {
        ...
        if(subannual){
          ...
          litter->ag_tree_inc+=-tinc_ind.leaf*pft->nind;  
        }
      }
