...
int main(int argc,char **argv)
{
  ...
  for(;i<argc;i++)
  {
    ...
    if(freadheader(item[count].file,&header,&swap,RESTART_HEADER,RESTART_VERSION))
    {
    ...
    fread(&item[count].header,sizeof(Header),1,item[count].file);
  ...
  header_offset=strlen(RESTART_HEADER)+sizeof(int)+sizeof(Header);
